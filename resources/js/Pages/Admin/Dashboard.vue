...
// Get Tailwind background class by schedule type
...
function onDayClick(day) {
    ...
    if (day.date) {
        ...
        const clickedDate = formatDate(d);
        selectedDateSched.value = clickedDate;

        allSchedBasedOnDate.value = props.schedules.filter(
            (sched) =>
                formatDate(new Date(sched.date)) === selectedDateSched.value
        );
    }
}

// Format time into 12-hour
...
const scheduleRows = computed(() =>
    [...props.schedules].sort((a, b) => (a.date < b.date ? 1 : -1))
);
    ...
    return (
        {
            ...
            prenatal: "bg-indigo-300 text-indigo-700",
            postnatal: "bg-yellow-300 text-yellow-700",
            timbang: "bg-rose-300 text-rose-700",
...
<template>
    ...
    <div class="flex flex-col h-full gap-4 overflow-hidden">
        ...
        <!-- Cards -->
        <div
            class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 flex-shrink-0"
        >
            <div
                v-for="(card, index) in cards"
                :key="index"
                class="flex items-center gap-3 p-4 rounded-2xl shadow-md border"
                :class="card.bg"
            >
                <div
        ...
        <div class="grid grid-cols-2 gap-4 overflow-hidden">
            ...
            >
                <h2
                    class="text-lg font-semibold text-slate-900 text-center mb-2 flex-shrink-0"
                >
                    Upcoming Schedules
                </h2>
        ...
        <!-- Recent Activity -->
        <div
            class="bg-white rounded-2xl shadow-md border border-slate-200 p-4 flex-shrink-0 overflow-hidden"
        >
            ...
            <div class="space-y-3 overflow-auto max-h-40">
                ...
                >
                    <div
                        class="mt-1 shrink-0 p-1 rounded border border-slate-300 flex items-center justify-center"
                    >
                        <component
                            :is="icons['ClipboardMinus']"
                            class="w-4 h-5 text-blue-700"
                        />
                    </div>
                    <div>
    ...
    <!-- Schedules by Date Modal -->
    <viewAllSchedBasedOnDate
        v-if="showAllSched"
        :schedules="allSchedBasedOnDate"
        :date="selectedDateSched"
