...
// Filtering logic (only partially immunized)
const filteredRecords = computed(() =>
    props.total
        .filter(rec => (rec.immunization_record || []).some(v => v.status === 'partially'))
        .filter((rec) => {
            const matchesSearch =
                !searchTerm.value ||
...
function pillClass(type) {
    return ({
        partially: "bg-green-300 text-green-900",
                    ...
                    <tbody>
                        <tr v-for="(record, index) in paginatedRecords" :key="record.id ?? index"
                            ...
                            <td class="p-3 truncate">
                                <div v-for="(vaccine, i) in record.immunization_record.filter(v => v.status === 'partially')"
                                    :key="i">
