<aside x-data="sidebar()" id="sidebar"
    ...
    <nav class="flex-1 flex flex-col justify-between p-4 font-semibold text-blue-700">
        <ul class="space-y-2 text-blue-700">
            ...
            <li>
                <a href="reports"
                    :class="[sidebarCollapsed ? 'pointer-events-none' : '', currentRoute === 'rhu/reports' ? 'bg-blue-600 text-white' : 'hover:bg-blue-600 hover:text-white']"
                    class="flex items-center gap-2 px-4 py-2 transition-all duration-300 rounded group">
                    <img :class="currentRoute === 'rhu/reports' ? 'filter brightness-0 invert' : 'group-hover:filter group-hover:brightness-0 group-hover:invert'"
